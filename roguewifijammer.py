...
@dataclass()
class ConfigRoguesWiFiJammer:
    data_size: int = 1472  # size od payload in b
    cw_min: int = 0  # min cw window size
    cw_max: int = 0  # max cw window size 1023 def
    r_limit: int = 7
    mcs: int = 7



class RogueWiFiJammer:
    def __init__(
            self,
            env: simpy.Environment,
            name: str,
            channel: dataclass,
            config: ConfigRoguesWiFiJammer = ConfigRoguesWiFiJammer(),
    ):
        self.config = config
