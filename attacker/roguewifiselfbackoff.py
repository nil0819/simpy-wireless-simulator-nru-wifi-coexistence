...
@dataclass()
class ConfigRoguesWiFiSelfBackoff:
    data_size: int = 1472  # size od payload in b
    cw_min: int = 3  # min cw window size
    cw_max: int = 3  # max cw window size 1023 def
    r_limit: int = 7
    mcs: int = 7



class RogueWiFiSelfBackoff:
    def __init__(
            self,
            env: simpy.Environment,
            name: str,
            channel: dataclass,
            config: ConfigRoguesWiFiSelfBackoff = ConfigRoguesWiFiSelfBackoff(),
    ):
        self.config = config
